...
async def fetch_incident_messages() -> list[dict[str, Any]]:
    ...
    """
    try:
        async with aiohttp.ClientSession() as session, session.get(MESSAGES_URL) as resp:
            if resp.status != 200:
                raise MvgApiError(f"Bad API call: Got response ({resp.status}) from {MESSAGES_URL}")
            if resp.content_type != "application/json":
                raise MvgApiError(f"Bad API call: Got content type {resp.content_type} from {MESSAGES_URL}")
            result = await resp.json()
    except aiohttp.ClientError as exc:
        raise MvgApiError(f"Bad API call: Got {type(exc)!s} from {MESSAGES_URL}") from exc
                    ...
                    "lines": [
                        {
                            ...
                            "transportType": _transport_type_value(line.get("transportType", ""))[0],
